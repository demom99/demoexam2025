...
Настроим DNS на BR-SRV. Вводите команду **по одному**, т.к. просит вписать пароль. Так же не забудьте поменять адреса по вашему варианту:
```yml
samba-tool dns add br-srv.au-team.irpo au-team.irpo hq-srv A 192.168.0.10 -U Administrator
...
samba-tool dns add br-srv.au-team.irpo au-team.irpo br-srv A 192.168.3.10 -U Administrator
samba-tool dns add br-srv.au-team.irpo au-team.irpo web.au-team.irpo A 172.16.2.1 -U Administrator
samba-tool dns add br-srv.au-team.irpo au-team.irpo docker.au-team.irpo A 172.16.1.1 -U Administrator
samba-tool dns query br-srv.au-team.irpo au-team.irpo @ ALL -U administrator
```

Создание группы `hq` и пользователей `hquser$` c паролем `P@ssw0rd`:
```yml
...
>лучше сделать под конец работы, иначе не сможете скачивать пакеты далее
```yml
...
Заходим в центр управления ► центр управления системой ► `Ethernet-интерфейсы` (Control center  ► system managenent center  ► `Ethernet-interfaces`).  Так же поменяйте сетевую подсистему на **`Etcnet`** в разделе Дополнительно (Advanced):
...
Как только удастся зайти за доменного пользователя, в терминал и работаем от **root**:
>можно попробовать без установки пакетов, у меня получилось
```yml
...
> здесь `192.168.2.0/27` адрес подсети, где находится HQ-CLI
```yml
apt-get update && apt-get install nfs-server nfs-utils -y
mkdir /raid/nfs
chmod 777 /raid/nfs
cp /etc/exports /etc/exports.back
echo "/raid/nfs 192.168.1.0/26(rw,no_subtree_check,no_root_squash)" >> /etc/exports
systemctl enable --now nfs-server
```

#### Переходим на HQ-CLI.
> здесь `192.168.0.10` - адрес HQ-SRV
```yml
...
```
Один уважаемый товарищ посоветовал ввести ещё эту команду (после неё, наверное нужно перезагрузить chronyd):
```yml
echo "allow 0.0.0.0/0" >> /еtc/chrony.conf
```
...
## Задание 5 (пока есть некоторые проблемы с HQ-CLI. после перезагрузки  HQ-CLI и BR-SRV ансибл перестал его пинговать)
...
#### По быстрому создаем sshuser на HQ-SRV, HQ-CLI, чтобы всё работало корректно

Создаем пользователя с идентификатором 2026 (-u) и принадлежностью к группе wheel (-G), добавляем право в файл **`/etc/sudoers`** и задаем ему пароль:
```yml
...
### Настройка службы ansible на BR-SRV

```yml
apt-get update && apt-get install nano ansible sshpass -y
```
...
```
Скачиваем необходимые пакеты. Советую вводить команды **по одному**
```yml
...
Сформируем файл инвентаря. **Обратите внимание на адреса**
```yml
...
```yml
apt-get update && apt-get install nano lamp-server -y
```
...
```yml
conf
ip nat source static tcp 192.168.0.10 80 172.16.1.2 8080
ip nat source static tcp 192.168.0.10 2026 172.16.1.2 2026
end
wr mem
```
...
Для проверки переходим на HQ-CLI, открываем браузер и вбиваем в поисковик docker.au-team.irpo
...
web.au-team.irpo не откроется, т.к. мы в файле конфигурации написали строчки web-based аутентификации
