...
**Полное доменное имя можно посмотреть в таблице для [Задания 9](https://github.com/Demomen-saveTF2/SA1-22demo2025/blob/main/Модуль_1.md#%D0%B7%D0%B0%D0%B4%D0%B0%D0%BD%D0%B8%D0%B5-9)**
...
<table align="center">
  ...
  <tr>
    ...
    <td align="center">172.16.1.1 - 172.16.1.14</td>
  </tr>
  <tr>
    <td align="center">ISP-BR</td>
    <td align="center">172.16.5.0/28</td>
    <td align="center">172.16.2.1 - 172.16.2.14</td>
  </tr>
</table>
...
- Настройте адресацию на интерфейсах:
  ...
  - Интерфейс, к которому подключен HQ-RTR, подключен к сети 172.16.1.0/28

  - Интерфейс, к которому подключен BR-RTR, подключен к сети 172.16.2.0/28
...
```yml
apt-get update && apt-get install iptables nano -y
...
> ip tunnel HQ-RTR BR-RTR mode gre
```yml
conf
int tunnel.0
...
> ip tunnel BR-RTR HQ-RTR mode gre
```yml
conf
int tunnel.0
...
- Обеспечьте защиту выбранного протокола посредством парольной защиты (не настроено)
...
> Настраивате на свой страх и риск, так как был случай, когда туннель ломалась при настройке защиты!
...
```
Настраиваем следующие параметры. **Обязательно раскомментируйте их!**
```bash
...
```yml
cp -r /etc/bind/zone/localdomain /etc/bind/zone/au-team.irpo
...
#### Настройка часового пояса на ISP
